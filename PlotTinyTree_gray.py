...
class regionHists:
	...
	# set a mass region hist for each btagging region
	self.allhists = []
	for key, val in CONF.bregions.items():
	    setattr(self, key, massregionHists(key, outputroot))
	    # for btagging regions in the signal, add a background region
	    self.allhists.append(key)
	    if key in CONF.signal_lst:
		bkg_name = CONF.bkg_lst[ CONF.signal_lst.index(key) ]
		setattr(self, bkg_name, bkgegionHists(bkg_name, outputroot, reweight))
		self.allhists.append(bkg_name)
        """
        # initiate all the massregion hists
        ...
        self.TwoTag_split_bkg  = bkgegionHists("Bkg_" + self.TwoTag_split.name, outputroot, reweight)
        self.ThreeTag_bkg      = bkgegionHists("Bkg_" + self.ThreeTag.name, outputroot, reweight)
        self.FourTag_bkg       = bkgegionHists("Bkg_" + self.FourTag.name, outputroot, reweight)
        """
    def Fill(self, event):
        """
        nb_j0 = 0
        ...
        """
        # figure out which region to put the event in
        # since regions are orthongonal, we can break after one succeeds
        b0 = [event.j0_trk0_Mv2, event.j0_trk1_Mv2]
        b1 = [event.j1_trk0_Mv2, event.j1_trk1_Mv2]
        for key, passfunc in CONF.bregions.items():
            if passfunc(b0, b1):
                getattr(self, key).Fill(event)
                break

        # now fill up backgrounds
        for key, passfunc in CONF.backgrounds.items():
            if passfunc(b0, event.j0_nTrk, b1, event.j1_nTrk):
                getattr(self, key).Fill(event)
        ...
        """
    def Write(self, outputroot):
        for histname in self.allhists:
            getattr(self,histname).Write(outputroot)
