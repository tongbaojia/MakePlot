...
try:
    ...
    import json        
from Xhh4bUtils.BkgFit.BackgroundFit_Flexible import BackgroundFit
...
# list of our signal regions
signal_lst = CONF.signal_lst
# and their backgrounds
bkg_lst = CONF.bkg_lst

# list of all cut regions
cut_lst = ["NoTag"] + CONF.cut_lst
dump_lst = cut_lst
...
def main():
	...
	bkg_lst = bkg_lst,
	output = inputpath + "Plot/", NRebin=1)
...
def fitestimation(histname="", inputdic={}):
    ...
    outroot.cd()
    est = {}
    # TODO: shouldn't this only run over the signal list????
    for i, cut in enumerate(cut_lst):
        #get the corresponding region
        cutcounts = {}
        for j, region in enumerate(region_lst):
		...
		if cut in signal_lst:
		    ref_cut = bkg_lst[signal_lst.index(cut) ]
		else:
		    ref_cut = "NoTag"	
		    #ref_cut = cut

            #start the temp calculation of Ftransfer
            if fitresult and cut in signal_lst:
		ind = signal_lst.index(cut)
                if ind < len(fitresult["mu" + histname.replace("_est", "")]):
                    Ftransfer = fitresult["mu" + histname.replace("_est", "")][ind]
                    Ftransfer_err = fitresult["mu" + histname.replace("_est", "") + "_e"][ind]
                    corr_temp = fitresult["corr_m"][ind]
                    Ftransfer_corr = corr_temp[ind + len(corr_temp)/2]
                    #print "cor is, ", fitresult["corr_m"], Ftransfer_corr, cut, histname, ind
                else:
...
def qcd_estimation(inputdic, histname=""):
    ...
    outroot.cd()
    est = {}
    # TODO: Again, shouldn't this just be the signal list???
    for i, cut in enumerate(cut_lst):
        #get the corresponding region
        cutcounts = {}
        for j, region in enumerate(region_lst):
