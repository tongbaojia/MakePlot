...
*/
   ...
   TFile *f = new TFile("hist-MiniNTuple.root");

   TH1F *inp_his = (TH1F *)f->Get("AllTag_Incl/leadHCand_Mass")->Clone(); 

   // negate the histogram
   TH1F *hSNR = new TH1F("neagtive", inp_his->GetTitle(), inp_his->GetXaxis()->GetNbins(), -1*inp_his->GetXaxis()->GetXmax(), -1*inp_his->GetXaxis()->GetXmin());
   // fill up the new one 
   const int nbins = inp_his->GetXaxis()->GetNbins();
   for (int i = 0; i <nbins; i ++) {
      hSNR->SetBinContent( i, inp_his->GetBinContent(nbins - i - 1) );
   }
   ...
   Double_t fr[2] = {-150, -50};
   Double_t sv[4], pllo[4], plhi[4], fp[4], fpe[4];
   //fr[0]=0.3*hSNR->GetMean();
   //fr[1]=3.0*hSNR->GetMean();

   pllo[0]=0.5; pllo[1]=-150.0; pllo[2]=1.0; pllo[3]=0.4;

   plhi[0]=25.0; plhi[1]=-50.0; plhi[2]=1000000.0; plhi[3]=25.0;

   sv[0]=5.0; sv[1]=-124.0; sv[2]=50000.0; sv[3]=1.0;
