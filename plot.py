...
def dumpRegion(config):
    ...
    if "TwoTag" or "OneTag" in config["cut"]:
        ...
        rebin_dic["j0_Pt"]      = array('d', [400, 450] + range(450, 600, 30) + range(600, 800, 40) + [800, 850, 900, 970, 1060, 1300, 2000])
        rebin_dic["j1_Pt"]      = array('d', range(250, 900, 50) + [900, 960, 1030, 1110, 1300, 2000])
        rebin_dic["trk0_Pt"]    = array('d', [0, 60] + range(60, 300, 30) + [300, 330, 360, 400, 450, 500, 570, 660, 800, 1300, 2000])
        rebin_dic["trk1_Pt"]    = array('d', range(0, 200, 20) + [200, 250, 400])
        rebin_dic["trk_dr"]     = array('d', [x * 0.1 for x in range(0, 10)] + [1, 1.2, 1.5, 2])
    ...
    if "ThreeTag" in config["cut"]:
        ...
        rebin_dic["j1_Pt"]      = array('d', range(250, 800, 50) + [800, 860, 930, 1000, 1100, 1300, 2000])
    ...
    if "FourTag" in config["cut"]:
        ...
        rebin_dic["trk1_Pt"]    = array('d', range(0, 180, 30) + [180, 400])
        rebin_dic["trk_dr"]     = array('d', [x * 0.1 for x in range(0, 10, 2)] + [1, 1.2, 1.5, 2])
